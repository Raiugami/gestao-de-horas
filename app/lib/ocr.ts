...
export function readOCRDate(text:string){
 // The date column can contain isolated OCR punctuation (for example "1 )").
 // Reject letters, slashes and additional digits so a header or project code is never a date.
 const match=text.trim().match(/^(\d{1,2})\s*[:.]?\s*[^\p{L}\p{N}/.,-]*$/u);
 return match?Number(match[1]):null;
}
export function resolveOCRDay(candidate:number|null,weekday:string|undefined,previousDay:number|undefined,month:string,rowAdjacent=true){
 ...
 // A distant row may share a weekday with the expected date; never pull it backward.
 if(!rowAdjacent)return null;
...
export async function recognizePage(canvas:HTMLCanvasElement,worker:Worker,month:string){
 ...
 for(let i=0;i<leftLines.length-1;i++){
  const line=leftLines[i];const dayToken=readOCRDate(line.text.trim().replace(/[|lI]/g,'1'));
  ...
  // Preserve an unreadable number as a candidate so a matching adjacent weekday can recover it.
  let day=dayToken;
  if(day!==null&&markers.length&&day!==markers.at(-1)!.day+1){
  ...
  const previous=markers.at(-1),y=line.bbox.y0/2,recent=markers.slice(-4),gaps=recent.slice(1).map((marker,index)=>marker.y-recent[index].y).filter(gap=>gap>0).sort((a,b)=>a-b),typicalGap=gaps.length?gaps[Math.floor(gaps.length/2)]:undefined,distance=previous?y-previous.y:null,rowAdjacent=previous===undefined||typicalGap===undefined||(distance!==null&&distance>0&&distance<=typicalGap*1.6);
  const resolvedDay=resolveOCRDay(day,weekday?.[1],previous?.day,month,rowAdjacent);if(resolvedDay===null)continue;
  markers.push({day:resolvedDay,word:resolveOCRWeekday(resolvedDay,weekday?.[1],month),y});
 }
